...
namespace
{
	...
	const static float FovDegree	= 10.0f;	// 視野角

	// near,farはなるべく小さくする
	const static float CameraNear	= 50.0f;	// near
	const static float CameraFar	= 100.0f;	// far

	// 視野角を狭めたら、表示範囲が狭い＝カメラがズームしたような見た目になるので
	// Z位置を下げる
	const static float CameraZPos	= -51.5f;	// Z位置
}
