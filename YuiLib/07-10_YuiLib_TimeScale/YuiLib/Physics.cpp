...
void Physics::Update()
{
	...
	for (auto& item : collidables)
	{
		...
		{
			velocity = VAdd(velocity, VGet(0, Gravity * timeScale, 0));
		...
		// ポジションに移動力を足す（タイムスケール考慮）
		auto scaledVelocity = VScale(velocity, timeScale);
		auto nextPos = VAdd(pos, scaledVelocity);
...
std::list<Collidable*> Physics::IsCollideLine(const VECTOR& start, const VECTOR& end) const
{
	...
	for (auto& obj : collidables)
	{
		...
		// ラインと円
		else if (kind == ColliderData::Kind::Circle2D)
		{
			auto circleColliderData = dynamic_cast<ColliderDataCircle2D*>(obj->colliderData);
...
void Physics::FixPosition()
{
	for (auto& item : collidables)
	{
		...
		// タイムスケールがかかる場合、velocityの力だけをタイムスケールの中間フレームとして
		// 補正しなおす
		VECTOR toFixedPos = VSub(item->nextPos, item->rigidbody.GetPos());
		toFixedPos = VScale(toFixedPos, 1.0f / timeScale);	// タイムスケールゼロだと呼ばれない
