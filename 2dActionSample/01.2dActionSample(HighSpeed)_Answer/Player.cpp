...
VECTOR CheckPlayerHitWithMap(Player& player, const Map& map, const VECTOR& velocity)
{
	...
	while (loop)
	{
		...
		for (int wIndex = 0; wIndex < MapWidth; wIndex++)
		{
			...
			{
				// マップチップなかったら以降を無視
				if (map.mapChips[wIndex][hIndex].chipKind == 0)
				{
					continue;
				}
...
bool IsHitPlayerWithMapChip(const Player& player, const VECTOR& futurePos, const MapChip& mapChip)
{
	...
	/////////////////////////////////////////////////////////////////
	// このままだと、速度が速いときに未来のポジションがマップチップより先に行ってしまい
	// 当たり判定をすり抜ける
	// 矩形同士の当たり判定
	...
	/////////////////////////////////////////////////////////////////
	// プレイヤー全ての四隅→プレイヤー未来の四隅の線分と、
	// マップチップ4辺の線分が交差していれば、当たっていることにする
	// 　※本来最適な２本を選べば４回計算しなくて良い点には注意
	/////////////////////////////////////////////////////////////////
	// Segment_Segment_MinLength 二つの線分の最近点間の距離を得る
	// ＝最近点（一番近い点）どうしの距離がゼロの時、交差していることになる
	/////////////////////////////////////////////////////////////////
	// 左上
	VECTOR LeftTop		 = VGet(player.pos.x - player.w * 0.5f, player.pos.y - player.h * 0.5f, 0);
	VECTOR LeftTopFuture = VGet(futurePos.x - player.w * 0.5f, futurePos.y - player.h * 0.5f, 0);
	if (CheckIsBoxLineCross(LeftTop, LeftTopFuture, mapChip.pos, mapChip.w, mapChip.h))
	{
		return true;
	}
	// 右上
	VECTOR RightTop		  = VGet(player.pos.x + player.w * 0.5f, player.pos.y - player.h * 0.5f, 0);
	VECTOR RightTopFuture = VGet(futurePos.x + player.w * 0.5f, futurePos.y - player.h * 0.5f, 0);
	if (CheckIsBoxLineCross(RightTop, RightTopFuture, mapChip.pos, mapChip.w, mapChip.h))
	{
		return true;
	}
	// 左下
	VECTOR LeftBottom		 = VGet(player.pos.x - player.w * 0.5f, player.pos.y + player.h * 0.5f, 0);
	VECTOR LeftBottomFuture  = VGet(futurePos.x - player.w * 0.5f, futurePos.y + player.h * 0.5f, 0);
	if (CheckIsBoxLineCross(LeftBottom, LeftBottomFuture, mapChip.pos, mapChip.w, mapChip.h))
	{
		return true;
	}
	// 右下
	VECTOR RightBottom		 = VGet(player.pos.x + player.w * 0.5f, player.pos.y + player.h * 0.5f, 0);
	VECTOR RightBottomFuture = VGet(futurePos.x + player.w * 0.5f, futurePos.y + player.h * 0.5f, 0);
	if (CheckIsBoxLineCross(RightBottom, RightBottomFuture, mapChip.pos, mapChip.w, mapChip.h))
	{
		return true;
	}
...
/// ボックスとラインの交差判定
/// </summary>
bool CheckIsBoxLineCross(const VECTOR& lineStart, const VECTOR& lineEnd, const VECTOR& boxCenterPos, float boxWidth, float boxHeight)
{
	VECTOR BoxLeftTop		= VGet(boxCenterPos.x - boxWidth * 0.5f, boxCenterPos.y - boxHeight * 0.5f, 0);
	VECTOR BoxRightTop		= VGet(boxCenterPos.x + boxWidth * 0.5f, boxCenterPos.y - boxHeight * 0.5f, 0);
	VECTOR BoxLeftBottom	= VGet(boxCenterPos.x - boxWidth * 0.5f, boxCenterPos.y + boxHeight * 0.5f, 0);
	VECTOR BoxRightBottom	= VGet(boxCenterPos.x + boxWidth * 0.5f, boxCenterPos.y + boxHeight * 0.5f, 0);

	// Segment_Segment_MinLength 二つの線分の最近点間の距離を得る
	// ＝最近点（一番近い点）どうしの距離がゼロの時、交差していることになる
	// floatの誤差を考えて0.001以下で許容する
	float fac = 0.001f;
	// 右辺
	float length = Segment_Segment_MinLength(lineStart, lineEnd, BoxRightTop, BoxRightBottom);
	if (length <= fac)
	{
		return true;
	}
	// 左辺
	length = Segment_Segment_MinLength(lineStart, lineEnd, BoxLeftTop, BoxLeftBottom);
	if (length <= fac)
	{
		return true;
	}
	// 上辺
	length = Segment_Segment_MinLength(lineStart, lineEnd, BoxLeftTop, BoxRightTop);
	if (length <= fac)
	{
		return true;
	}
	// 下辺
	length = Segment_Segment_MinLength(lineStart, lineEnd, BoxLeftBottom, BoxRightBottom);
	if (length <= fac)
	{
		return true;
	}
