...
private:
	...
	void AnalyzeWallAndFloor(MV1_COLL_RESULT_POLY** Wall, MV1_COLL_RESULT_POLY** Floor, int& WallNum, int& FloorNum, MV1_COLL_RESULT_POLY_DIM HitDim);

	// 壁ポリゴンとの当たりをチェックし、補正すべき移動ベクトルを返す
	VECTOR CheckHitWithWall(MV1_COLL_RESULT_POLY** Wall, int WallNum, const VECTOR& NowPos, bool IsMove);

	// 床ポリゴンとの当たりをチェックし、補正すべき移動ベクトルを返す
	VECTOR CheckHitWithFloor(MV1_COLL_RESULT_POLY** Floor, int FloorNum, const VECTOR& NowPos, bool IsMove);
